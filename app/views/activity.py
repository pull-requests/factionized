...
from app.models import Game, Thread, Activity, Message, Vote
...
def activities(request, game_id, round_id, thread_id):
    return get_activities(Activity, request.user, thread_id)

@login_required
def votes(request, game_id, round_id, thread_id):
    if request.method == 'GET':
        return get_activities(Vote, request.user, thread_id)
